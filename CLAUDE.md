...
- **Every PR description MUST include a Test Plan with evidence — and, for any user-facing/UI change, an embedded screenshot. Not optional.** Document exactly how the change was verified: each automated check run (`tsc`/lint, CI, the e2e at `test/e2e/preview-flow.mjs`) with its pass/fail result, plus a one-line note on manual/browser verification.
  - **Screenshots are required for UI changes — and must be *embedded and visible* in the PR, not just described.** Before→after when altering an existing surface. A UI PR without a visible screenshot is not done.
  - **How to embed (this is the followable mechanism — no excuses):** capture the screenshot (the Playwright MCP browser, saved to a file), then commit it to the branch under `docs/pr-shots/<short-name>.png` and embed it in the PR body with `![caption](https://raw.githubusercontent.com/bz28/visage/<commit-sha>/docs/pr-shots/<short-name>.png)`. Use the **commit SHA** (from `git rev-parse HEAD` after committing the image) so the URL stays valid after the branch is deleted. The repo is public, so the raw URL renders. Verify it renders: `curl -s -o /dev/null -w '%{http_code}' "<url>"` should be `200`, and the PR body should show the image.
  - Only if a surface **genuinely** can't be captured here (e.g. mobile with no simulator, a sub-second transient) may you skip the image — and then you must **say so explicitly** in the Test Plan with what you verified instead. Never silently omit it.
  - The same evidence (including the screenshot link) belongs in the message you give the user when reporting the PR.
- **Verify the blast radius, not just the new thing.** A change to a shared component, hook, API contract, or the landmark/composite/simulation pipeline can silently break surfaces it didn't obviously touch. Identify what the change could affect (every call site of an edited component/function, every screen that reads a changed value) and confirm those *still function the same* — exercise the adjacent flow, not just the new feature. **Embed a screenshot of each affected surface you checked** (same `docs/pr-shots/` + raw-URL mechanism above), not only the changed one. If you can't verify an affected surface, list it as an explicit risk in the Test Plan rather than assuming it's fine.
